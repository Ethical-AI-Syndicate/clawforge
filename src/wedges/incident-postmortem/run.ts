 ...
 *
 * Orchestrates a complete incident postmortem record using the ClawForge
 * kernel.  Identical call sequence to Ship a Change — only the banner
 * text and completion summary differ.
 *
 * Usage:
 *   node dist/wedges/incident-postmortem/run.js \
 *     --intent <incident-intent.json> \
...
async function main(): Promise<void> {
  ...
  log("Incident Postmortem — ClawForge Wedge");
  log("======================================");
  log(`Incident: ${intent.title}`);
  log(`Lead:     ${args.actorId}`);
  log(`Host:     ${args.hostId}`);
  log(`Evidence: ${args.artifactPaths.length} artifact(s)`);
  log("");
  ...
  try {
    ...
    log(`[1/5] Postmortem run created: ${runId}`);
    ...
    log(`[2/5] Incident record filed: ${intent.intentId}`);
    ...
    for (const filePath of args.artifactPaths) {
      ...
      artifactCount++;
      log(`[3/5] Evidence ${artifactCount}/${args.artifactPaths.length}: ${label} (${record.size} bytes, ${record.sha256.slice(0, 12)}…)`);
    }

    // --- 5. RunCompleted (postmortem closed) ----------------------------------
    ...
    eventStore.appendEvent(runId, {
      ...
      actor,
      payload: {
        summary: `Postmortem closed: ${intent.title}. ${artifactCount} evidence artifact(s) attached.`,
      },
    });
    log(`[4/5] Postmortem closed`);
